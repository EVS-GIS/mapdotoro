...
#' Create continuity_width_full_side_geom view
#'
#' @param db_con database connection parameters.
#' @param view_name view name.
#'
...
#' @return text
#' @export
continuity_width_full_side_view_geom <- function(db_con,
                                                 view_name = "continuity_width_full_side_geom",
                                                 continuity_table_name){
  query <- glue::glue("
    DO $$
    DECLARE
        continuity text;
    ...
    BEGIN
		...
		SELECT
				hydro_swaths.gid, hydro_swaths.axis, hydro_swaths.measure_from_outlet,
				hydro_swaths.strahler, hydro_swaths.gid_region, {continuity_table_name}_full_side.*,
				ST_SetSRID(hydro_swaths.geom, 4326) AS geom
			FROM hydro_swaths
			LEFT JOIN (
                  SELECT
        ...
        -- Constructing the SELECT part of the query with SUM left and right side
        FOR continuity IN (SELECT column_name FROM information_schema.columns
    						WHERE table_name = '{continuity_table_name}'
    							AND column_name NOT IN ('axis', 'measure_medial_axis',
    												'side', 'id', 'hydro_swaths_gid'))
    	-- Concatenate the query
        LOOP
        ...
        query := query || '
                  ...
                  GROUP BY
                      axis,
                      measure_medial_axis,
                      hydro_swaths_gid) AS {continuity_table_name}_full_side
					  	ON {continuity_table_name}_full_side.hydro_swaths_gid = hydro_swaths.gid';
